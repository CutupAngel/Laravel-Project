...
<div class="app-card app-card-orders-table h-100 shadow-sm">
  <div class="app-card-header p-3">
    <div class="row justify-content-between align-items-center">
      <div class="col-auto">
        <h4 class="app-card-title">Users List</h4>
      </div>
      <div class="col-auto">
        <a href="{{route('department_account_create')}}" class="btn app-btn-primary">
  ...
  <div class="app-card-body p-4">
    ...
    <div class="table-responsive">
      <table id="datatable_users" class="table app-table-hover mb-0 text-left">
        <thead>
          <tr>
        ...
        <tbody>
          ...
          <tr>
            <td class="cell truncate ps-3">{{$user->name}}</td>
            <td class="cell truncate">{{$user->email}}</td>
            <td class="cell">
              @if($user->role == 1)
              <span class="badge bg-secondary"> Owner </span>
              @elseif($user->role == 2)
              <span class="badge bg-primary"> Admin Account </span>
              @elseif($user->role == 4)
              <span class="badge bg-info"> Staff </span>
              @else
              <span class="badge bg-success"> Account Department </span>
              @endif
            </td>
            <td class="cell">
              <span>
                <?php
                echo date("dS M", strtotime($user->created_at));
                ?>
              </span>
              <span class="note">
                <?php
            ...
            <td class="cell pe-3 pe-md-3">
              <a class="btn-sm app-btn-secondary me-2" href="{{ route('department_account_edit',$user->id) }}">
              ...
              <a class="btn-sm app-btn-secondary border-danger text-danger" href="{{ route('department_account_delete',$user->id) }}">
